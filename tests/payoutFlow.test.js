...
const payoutTests = [
  {
    ...
    async run() {
      ...
      global.fetch = async (url, options = {}) => {
        ...
        if (String(url).includes('/validation/bankDetails')) {
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ data: { status: 'VALID' } })
          }
        }

        if (String(url).includes('/addBeneficiary')) {
          const payload = JSON.parse(options.body || '{}')
          return {
            ok: true,
            status: 200,
            text: async () => JSON.stringify({ data: { beneId: payload.beneId, status: 'ACTIVE' } })
          }
        }
  ...
  },
  {
    ...
    async run() {
      ...
      global.fetch = async (url) => {
        ...
        if (String(url).includes('/requestAsyncTransfer')) {
          return {
            ok: true,
            status: 200,
